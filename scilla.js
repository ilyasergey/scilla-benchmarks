...
const runLocalInterpreterAsync = async (cmdOptions, outputPath) => {
  ...
  if (!fs.existsSync(SCILLA_BIN_PATH)) {
    ...
    throw new InterpreterError(
      "Kaya RPC Runtime Error: Scilla-runner not found"
    );
  }
...
module.exports = {
  ...
  executeScillaRun: async (
    ...
    const cmdOpt = [].concat.apply(
      [],
      [standardOpt, initOpt, outputOpt, codeOpt, blockchainOpt]
    );
    ...
    if (isCodeDeployment) {
      ...
      logVerbose(
        logLabel,
        `Calling transition within contract ${payload.toAddr}`
      );
