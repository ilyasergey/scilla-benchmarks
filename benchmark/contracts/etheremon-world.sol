...
contract EtheremonDataBase is EtheremonEnum, BasicAccessControl, SafeMath {
    ...
    // write
    function withdrawEther(address _sendTo, uint _amount) public returns(ResultCode);
    ...
    function setMonsterClass(uint32 _classId, uint256 _price, uint256 _returnPrice, bool _catchable)  public returns(uint32);
    function addMonsterObj(uint32 _classId, address _trainer, string memory _name)  public returns(uint64);
    function setMonsterObj(uint64 _objId, string memory _name, uint32 _exp, uint32 _createIndex, uint32 _lastClaimIndex) public;
    ...
    function removeMonsterIdMapping(address _trainer, uint64 _monsterId)  public;
    function addMonsterIdMapping(address _trainer, uint64 _monsterId)  public;
    function clearMonsterReturnBalance(uint64 _monsterId)  public returns(uint256 amount);
    function collectAllReturnBalance(address _trainer)  public returns(uint256 amount);
    function transferMonster(address _from, address _to, uint64 _monsterId)  public returns(ResultCode);
    function addExtraBalance(address _trainer, uint256 _amount)  public returns(uint256);
    function deductExtraBalance(address _trainer, uint256 _amount)  public returns(uint256);
    function setExtraBalance(address _trainer, uint256 _amount) public;
    
    // read
    function getSizeArrayType(ArrayType _type, uint64 _id) view public returns(uint);
    function getElementInArrayType(ArrayType _type, uint64 _id, uint _index) view public returns(uint8);
    function getMonsterClass(uint32 _classId) view public returns(uint32 classId, uint256 price, uint256 returnPrice, uint32 total, bool catchable);
    function getMonsterObj(uint64 _objId) view public returns(uint64 objId, uint32 classId, address trainer, uint32 exp, uint32 createIndex, uint32 lastClaimIndex, uint createTime);
    function getMonsterName(uint64 _objId) view public returns(string memory name);
    function getExtraBalance(address _trainer) view public returns(uint256);
    function getMonsterDexSize(address _trainer) view public returns(uint);
    function getMonsterObjId(address _trainer, uint index) view public returns(uint64);
    function getExpectedBalance(address _trainer) view public returns(uint256);
    function getMonsterReturn(uint64 _objId) view public returns(uint256 current, uint256 total);
}
...
contract EtheremonGateway is EtheremonEnum, BasicAccessControl {
    ...
    // read 
    function isGason(uint64 _objId) view external returns(bool);
    function getObjBattleInfo(uint64 _objId) view external returns(uint32 classId, uint32 exp, bool isGason, 
        uint ancestorLength, uint xfactorsLength);
    function getClassPropertySize(uint32 _classId, PropertyType _type) view external returns(uint);
    function getClassPropertyValue(uint32 _classId, PropertyType _type, uint index) view external returns(uint32);
}
...
contract EtheremonWorld is EtheremonGateway, SafeMath {
    ...
    function getEarningAmount() view public returns(uint256) {
    ...
    function withdrawEther(address payable _sendTo, uint _amount) onlyModerators external returns(ResultCode) {
        if (_amount > address(this).balance) {
            emit EventWithdrawEther(_sendTo, ResultCode.ERROR_INVALID_AMOUNT, 0);
        ...
        emit EventWithdrawEther(_sendTo, ResultCode.SUCCESS, _amount);
    ...
    // helper
    function getRandom(uint8 maxRan, uint8 index, address priAddress) view public returns(uint8) {
    ...
    function getObjIndex(uint64 _objId) view public returns(uint32 classId, uint32 createIndex, uint32 lastClaimIndex) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterObjAcc memory obj;
        (obj.monsterId, obj.classId, obj.trainer, obj.exp, obj.createIndex, obj.lastClaimIndex, obj.createTime) = data.getMonsterObj(_objId);
    ...
    // only gen 0
    function getGen0COnfig(uint32 _classId) view public returns(uint32, uint256, uint32) {
        Gen0Config storage gen0 = gen0Config[_classId];
        return (gen0.classId, gen0.originalPrice, gen0.total);
    }
    
    // only gen 0
    function getReturnFromMonster(uint64 _objId) view public returns(uint256 current, uint256 total) {
        /*
    ...
    function moveDataContractBalanceToWorld() external {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        data.withdrawEther(address(this), address(data).balance);
    }
    
    function renameMonster(uint64 _objId, string calldata name) isActive external {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterObjAcc memory obj;
        (obj.monsterId, obj.classId, obj.trainer, obj.exp, obj.createIndex, obj.lastClaimIndex, obj.createTime) = data.getMonsterObj(_objId);
    ...
    function catchMonster(uint32 _classId, string calldata _name) isActive external payable {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterClassAcc memory class;
        (class.classId, class.price, class.returnPrice, class.total, class.catchable) = data.getMonsterClass(_classId);
        ...
        emit EventCatchMonster(msg.sender, objId);
    }
    ...
    function cashOut(uint256 _amount) public returns(ResultCode) {
        ...
        if (address(this).balance + address(data).balance < _amount){
            revert();
        ...
        if (_amount > 0) {
            ...
            if (!msg.sender.send(_amount)) {
                ...
                emit EventCashOut(msg.sender, ResultCode.ERROR_SEND_FAIL, 0);
                return ResultCode.ERROR_SEND_FAIL;
            }
        }
        
        emit EventCashOut(msg.sender, ResultCode.SUCCESS, _amount);
        return ResultCode.SUCCESS;
    }
    
    // read access
    
    function getTrainerEarn(address _trainer) view public returns(uint256) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
    ...
    function getTrainerBalance(address _trainer) view external returns(uint256) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
    ...
    function getMonsterClassBasic(uint32 _classId) view external returns(uint256, uint256, uint256, bool) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterClassAcc memory class;
        (class.classId, class.price, class.returnPrice, class.total, class.catchable) = data.getMonsterClass(_classId);
