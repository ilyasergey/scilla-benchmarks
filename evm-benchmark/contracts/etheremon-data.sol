...
contract EtheremonData is EtheremonDataBase {
    ...
    // write access
    function withdrawEther(address payable _sendTo, uint _amount) onlyOwner public returns(ResultCode) {
    ...
    function addMonsterObj(uint32 _classId, address _trainer, string memory _name) onlyModerators public returns(uint64) {
        MonsterClass storage class = monsterClass[_classId];
        if (class.classId == 0)
            return 0;
    ...
    function setMonsterObj(uint64 _objId, string memory _name, uint32 _exp, uint32 _createIndex, uint32 _lastClaimIndex) onlyModerators public {
